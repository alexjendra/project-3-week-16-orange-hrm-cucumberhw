...
public class UserTestPageSteps {
    ...
    public void iShouldSeeTheAddUserText(String expectedMessage) {
        Assert.assertEquals(new AddUserPage().verifyAddUserText(), expectedMessage);
    }
    @And("I select User Role as Admin")
    public void iSelectUserRoleAsAdmin() {
        new AddUserPage().mouseHoverAndClickOnUserRoleDropDownInAddUser();
        new AddUserPage().mouseHoverAndCLickOnAdminOptionInUserRoleDropDownInAddUser();
    }

    @And("I enter Employee Name {string}")
    public void iEnterEmployeeName(String eName) {
        new AddUserPage().sendDataToEmployeeNameFieldInAddUser(eName);
    }

    @And("I enter User Name {string}")
    public void iEnterUserName(String uName) {
        new AddUserPage().sendDataToUserNameFieldInAddUser(uName);
    }

    @And("I select Status as Disable")
    public void iSelectStatusAsDisable() {
        new AddUserPage().clickOnStatusDropDownMenuInAddUser();
        new AddUserPage().mouseHoverAndClickOnDisabledOptionInAddUser();
    }

    @And("I enter Password {string}")
    public void iEnterPassword(String pass) {
        new AddUserPage().sendDataToPasswordFieldInAddUser(pass);
    }

    @And("I enter Confirm Password {string}")
    public void iEnterConfirmPassword(String cPass) {
        new AddUserPage().sendDataToConfirmPasswordFieldInAddUser(cPass);
    }

    @And("I click on Save button")
    public void iClickOnSaveButton() {
        new AddUserPage().clickOnSaveButtonInAddUser();

    }

    @Then("I should see the message {string}")
    public void iShouldSeeTheMessage(String expectedMessage) {
        Assert.assertEquals(new AddUserPage().getSuccessfullySavedMessageInAddUser(), expectedMessage);
    }
